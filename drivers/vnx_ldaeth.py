...
''' 
    ...
    JA 12/12/2024    Updated version of Ethernet CLI app
                ...
                if(ipaddress !=''):
                    if not attobj.check_deviceexists(ctypes.c_char_p(ipaddress)):
                        for index in range(0, 5):
                            startattenint = 10 + (index*2)
                            stepint = 3  + (index*2)
                            endattenint = 50  + (index*2)
                            delay = 1000  + (index*2)
                            print('index:',index)
                            attobj.set_rampstartattenuation(ctypes.c_char_p(ipaddress), startattenint)
                            attobj.set_attenuationstep(ctypes.c_char_p(ipaddress), stepint)
                            attobj.set_rampendattenuation(ctypes.c_char_p(ipaddress), endattenint)
                            attobj.set_profiledwelltime(ctypes.c_char_p(ipaddress), delay)
                            attobj.set_dwelltime(ctypes.c_char_p(ipaddress), delay)
                            attobj.set_idletime(ctypes.c_char_p(ipaddress), 0)
                            attobj.set_holdtime(ctypes.c_char_p(ipaddress), 1)
                            attobj.set_rampbidirectional(ctypes.c_char_p(ipaddress), False)
                            attobj.set_rampmode(ctypes.c_char_p(ipaddress), 2)
                    else:
                        print ("Device not exists.. Please Check!")
                else:
                    print ("Device IP missing.. Please Check!")
