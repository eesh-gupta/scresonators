...
def _perform_initial_scan(hw, expt_path, result, freq_idx, power, att, fname, config):
    ...
    # Calculate photon number directly in constructor
    pin = (
        power
        - result.config["att"]
    ...
    # Create and return measurement object with direct calculations
    return ResonatorMeasurement(
        ...
        kappa=kappa,
        photon_number=n(pin, freq_center, q_total, fit_params[2] * 1e4),
        averages=1,
...
def _should_stop_measuring(result, freq_idx, next_time):
    ...
    """
    thresh = [0.02, 0.005, -0.05, -0.15]  # Simplified threshold selection
