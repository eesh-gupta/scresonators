...
def get_best_fit(
    ...
    if override is not None and override in check_measures:
        ...
        if fitfunc is not None:
            ydata = [
                data[check] for check in check_measures
            ...
            for fit, fit_err_check in zip(fits, fit_errs):
                par_error_norm.append(
                    np.mean(np.sqrt(np.abs(np.diag(fit_err_check))) / np.abs(fit))
                )
...
def fitdecayslopesin(xdata, ydata, fitparams=None, debug=False):
    ...
    try:
        pOpt, pCov = sp.optimize.curve_fit(
            decayslopesin, xdata, ydata, p0=fitparams, bounds=bounds
        )
        # return pOpt, pCov
    except RuntimeError:
        try:
...
def fithanger(xdata, ydata, fitparams=None):
    ...
    try:
        pOpt, pCov = sp.optimize.curve_fit(
            hangerS21func, xdata, ydata, p0=fitparams, bounds=bounds
        )
        # print(pOpt)
        pOpt[1] = pOpt[1]
        pOpt[2] = pOpt[2]
