...
class ZNB20(SocketInstrument):
    ...
    def __init__(self, name="BatMouse", address=None, enabled=True, **kwargs):
        SocketInstrument.__init__(
            self, name, address, enabled=enabled, recv_length=2**20, **kwargs
        )
    ...
    def initialize_one_tone_spectroscopy_seg(self, traces, Sparams):

        # Linear sweep in frequency
        self.set_sweeptype("segm")
    ...
    def create_traces(self, traces, Sparams):
        ...
        for trace, Sparam in zip(traces, Sparams):
            self.write('calc:parameter:sdef  "%s","%s"' % (trace, Sparam))
        ...
        for i, trace in enumerate(traces):
            self.write('disp:wind1:trac%s:feed "%s"' % (i + 1, trace))
    ...
    def _get_data(self, trace, data_format="db-phase"):
        ...
        if data_format.lower() == "real-imag":
            ...
            try:
                return 20.0 * np.log10(abs(real + 1j * imag)), np.angle(
                    real + 1j * imag
                )
            except RuntimeError:
            ...
            try:
                return abs(real + 1j * imag) ** 2.0, np.angle(real + 1j * imag)
            except RuntimeError:
                print("Division by zero error - Amplitude")
                return np.ones_like(real)
        else:
            raise ValueError(
                "data-format must be: 'real-imag', 'db-phase', 'amp-phase'."
            )
    ...
    def measure(self):
        ...
        """
        logging.info(__name__ + " : start to measure and wait until it is finished")
    ...
    def set_sweeptype(self, sweeptype="LIN"):
        ...
        if sweeptype.upper() in ("LIN", "LOG", "POW", "CW", "POIN", "SEGM"):
            ...
            raise ValueError(
                "set_sweeptype(): can only set LIN, LOG, POW, CW, POIN or SEGM"
            )
    ...
    def define_segment(
        ...
        """
        logging.debug(
            __name__ + " : we are defining the segment number %s" % segment_number
        )
        ...
        if set_time == "dwell":
            ...
            self.write(
                "SEGM%s:DEF %sGHZ,%sGHZ,%s,%sDBM,%sS,%s,%sHZ"
                % (
                    segment_number,
                    startfrequency / 1e9,
                    stopfrequency / 1e9,
            ...
            asked_freq_start = self.query("SEGM%s:FREQ:STAR?" % segment_number)[:-1]
            given_freq_start = str(startfrequency)

            # if asked_freq_start != given_freq_start:
            #     print("error in setting start frequency at %s" % startfrequency)
            #     print(asked_freq_start, given_freq_start)

            asked_freq_stop = self.query("SEGM%s:FREQ:STOP?" % segment_number)[:-1]
            given_freq_stop = str(stopfrequency)

            # if asked_freq_stop != str(given_freq_stop):
            #     print("error in setting stop frequency at %s" % stopfrequency)
            #     print(asked_freq_stop, given_freq_stop)

            if self.query("SEGM%s:SWE:POIN?" % segment_number)[:-1] != str(points):
                print("error in setting number of points")

            asked_power = self.query("SEGM%s:POW?" % segment_number)[:-1]
            given_power = str(power)
            if asked_power != given_power:
                print("error in setting power")
                print(asked_power, given_power)

            # asked_dwell = self.query("SEGM%s:SWE:DWEL?" % segment_number)[:-1]
            # given_dwell = str(time)
            # if asked_dwell != given_dwell:
            #     print("error in setting dwell time")
            #     print(asked_dwell, given_dwell)

            if self.query("SEGM%s:BWID?" % segment_number)[:-1] != str(BW):
            ...
            print("Freq start is")
            # print( self.query('SEGM%s:FREQ:STAR?'% segment_number))
            # if self.query('SEGM%s:FREQ:STAR?'% segment_number) != str(startfrequency*1e9, 'utf-8'):
            #     print('error in setting start frequency at %s' %startfrequency)
            #     # print 'set at %s' %freq_start/1e9

            if self.query("SEGM%s:FREQ:STOP?" % segment_number) != str(
                stopfrequency * 1e9, "utf-8"
            ):
                print("error in setting stop frequency at %s" % stopfrequency)
    ...
    def define_power_sweep(
        ...
        logging.debug(
            __name__
            + " : making a sweep in power from %s to %s with a step of %s"
            % (startpow, stoppow, steppow)
        )
        ...
        for i in np.arange(point):
            self.define_segment(
                i + 1, cwfrequency, cwfrequency, 1, pow_vec[i], time, BW, set_time
            )
    ...
    def set_centerfrequency(self, centerfrequency=1.0):
        """
    ...
    def set_frequencyspan(self, frequencyspan=1.0):
        """
    ...
    def set_startfrequency(self, startfrequency=1.0):
        """
    ...
    def set_stopfrequency(self, stopfrequency=1.0):
        """
        Set the start frequency of the instrument
    ...
    def set_cwfrequency(self, cwfrequency=1.0):
        """
    ...
    def get_power(self):
        ...
        return self.query("source:power?").strip()

    def set_startpower(self, startpower=-40.0):
        """
    ...
    def set_stoppower(self, stoppower=-40.0):
        """
    ...
    def set_measBW(self, measBW=1000.0):
        """
