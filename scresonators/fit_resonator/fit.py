...
from . import cavity_functions as ff
from . import plot as fp
from . import resonator as res
...
def extract_near_res(
    ...
    if len(y_temp) < 5:
        print(
            "Less than 5 Data points to fit data, not enough points near",
...
def find_initial_guess(x, y1, y2, Method, output_path, plot_extra):
    ...
    try:
        ...
        print(
            ">Problem initializing data in find_initial_guess(), please make",
    ...
    try:
        ...
        print(
            ">Problem in function find_circle, please make sure data is of ",
            "correct format",
        )
        quit()
    ...
    try:
        if f_c < 0:
            print(
                ">Resonance frequency is negative. Please only input ",
    ...
    if Method.method == "DCM" or Method.method == "PHI":
        try:
            ...
            if Method.method == "DCM":
                print(
                    ">Failed to find initial guess for method DCM.",
        ...
        try:
            ...
            # (this is where Qi and Qc are actually guessed)
            popt, pcov = spopt.curve_fit(
                ff.one_cavity_peak_abs_REFLECTION,
                x,
                np.abs(ydata),
                p0=[Q, Qc, f_c],
                bounds=(0, [np.inf] * 3),
            ...
            print(
                ">Failed to find initial guess for method DCM REFLECTION. ",
                "Please manually initialize a guess",
            )
            quit()
        ...
        try:
            ...
            print(
                ">Failed to find initial guess for method INV. ",
                "Please manually initialize a guess",
            )
            quit()
        ...
        try:
            ...
            print(
                ">Failed to find initial guess for method CPZM. ",
                "Please manually initialize a guess",
            )
            quit()
    else:
        print(
            ">Method is not defined. Please choose a method: DCM, ",
...
def monte_carlo_fit(xdata=None, ydata=None, parameter=None, Method=None):
    ...
    try:
        ydata_1stfit = Method.func(
            xdata, *parameter
        )  # set of S21 data based on initial guess parameters

        ## weight condition
        if Method.MC_weight == "yes":
            weight_array = 1 / abs(ydata)  # new array of inversed magnitude ydata
        else:
            weight_array = np.full(
                len(xdata), 1
            )  # new array of len(xdata) all slots filled with 1

        weighted_ydata = np.multiply(
            weight_array, ydata
        )  # array filled with 1s if MC_weight='yes' and exact same array as ydata otherwise
        weighted_ydata_1stfit = np.multiply(
            weight_array, ydata_1stfit
        )  # array with values (ydata^(-1))*ydata_1stfit if MC_weight='yes' and exact same array as ydata_1stfit otherwise
        error = np.linalg.norm(weighted_ydata - weighted_ydata_1stfit) / len(
            xdata
        )  # first error #finds magnitude of (weighted_ydata-weighted_ydata_1stfit) and divides by length (average magnitude)
        error_0 = error
    except:
    ...
    try:
        while counts < Method.MC_rounds:
            ...
            random = Method.MC_step_const * (
                np.random.random_sample(len(parameter)) - 0.5
            )
    ...
    if error < error_0:
        ...
        if Method.manual_init != None:
            print(
                ">User input parameters getting stuck in local minimum, please input more accurate parameters"
            )
    else:
        stop_MC = True
...
def fit_phase(f_data, z_data, guesses=None):
    ...
    p_final = spopt.leastsq(
        residuals_full, [fr_guess, Ql_guess, theta_guess, delay_guess]
    )
...
def preprocess_linear(
    ...
    angle = np.subtract(
        angle, intercept
    )  # rotate off resonant point to (1,0i) in complex plane
    y_test = np.multiply(np.abs(ydata), np.exp(1j * angle))
    if plot_extra:
    ...
    if plot_extra:
        fp.plot(
            np.real(preprocessed_data),
            np.imag(preprocessed_data),
            "Normalize_4",
            output_path,
        )
...
def background_removal(
    databg, linear_amps: np.ndarray, phases: np.ndarray, output_path: str
):
    x_bg = databg.freqs
    ...
    fp.plot2(
        databg.freqs, databg.linear_amps, x_bg, linear_amps_bg, "VS_mag", output_path
    )
...
def min_fit(params, xdata, ydata, Method):
    ...
    try:
        if Method.method == "DCM" or Method.method == "PHI":
            minner = lmfit.Minimizer(
                ff.min_one_Cavity_dip, params, fcn_args=(xdata, ydata)
            )
        elif Method.method == "DCM REFLECTION":
            minner = lmfit.Minimizer(
                ff.min_one_Cavity_DCM_REFLECTION, params, fcn_args=(xdata, ydata)
            )
        elif Method.method == "INV":
            minner = lmfit.Minimizer(
                ff.min_one_Cavity_inverse, params, fcn_args=(xdata, ydata)
            )
        elif Method.method == "CPZM":
            minner = lmfit.Minimizer(
                ff.min_one_Cavity_CPZM, params, fcn_args=(xdata, ydata)
            )
        else:
            print(
                ">Method is not defined. Please choose a method: DCM, ",
    ...
    try:
        ...
        if (
            Method.method == "DCM"
            or Method.method == "PHI"
            or Method.method == "DCM REFLECTION"
        ):
            ...
            if "Q" in p_names and "Qc" in p_names and "Qi" not in Method.MC_fix:
                if Method.method == "PHI":
                    Qi = (
                        (ci["Q"][1][1]) ** -1
                        - np.abs(ci["Qc"][1][1] ** -1 * np.exp(1j * fit_params[3]))
                    ) ** -1
                    Qi_neg = (
                        Qi
                        - (
                            (ci["Q"][0][1]) ** -1
                            - np.abs(ci["Qc"][2][1] ** -1 * np.exp(1j * fit_params[3]))
                        )
                        ** -1
                    )
                    Qi_pos = (
                        ...
                        - (
                            (ci["Q"][2][1]) ** -1
                            - np.abs(ci["Qc"][0][1] ** -1 * np.exp(1j * fit_params[3]))
                        )
                        ** -1
                    )
                else:
                    Qi = (
                        (ci["Q"][1][1]) ** -1
                        - np.real(ci["Qc"][1][1] ** -1 * np.exp(1j * fit_params[3]))
                    ) ** -1
                    Qi_neg = (
                        Qi
                        - (
                            (ci["Q"][0][1]) ** -1
                            - np.real(ci["Qc"][2][1] ** -1 * np.exp(1j * fit_params[3]))
                        )
                        ** -1
                    )
                    Qi_pos = (
                        ...
                        - (
                            (ci["Q"][2][1]) ** -1
                            - np.real(ci["Qc"][0][1] ** -1 * np.exp(1j * fit_params[3]))
                        )
                        ** -1
                    )
            ...
            if "Qc" in p_names:
                ...
                if np.isinf(Qc_Re_conf):
                    Qc_Re_conf = min(
                        np.abs(Qc_Re - Qc_Re_neg), np.abs(Qc_Re - Qc_Re_pos)
                    )
            else:
                Qc_conf = 0
                Qc_Re_conf = 0
...
def fit(resonator):
    ...
    if resonator.databg is not None:
        ...
        t_ydata, t_slope, t_intercept, t_slope2, t_intercept2 = preprocess_linear(
            xdata, ydata, normalize, output_path, plot_extra
        )
        ...
        # else:
        ydata, slope, intercept, slope2, intercept2 = (
            t_ydata,
            t_slope,
            t_intercept,
            t_slope2,
        ...
        ydata = preprocess_circle(
            xdata, ydata, output_path, plot_extra, fix_freq=fix_freq
        )
    else:
        pass
    ...
    change_Q, change_Qi, change_Qc, change_w1, change_phi, change_Qa = (
        True,
        True,
        True,
        True,
    ...
    if len(x_raw) < 20:
        print(
            ">Not enough data points to run code. Please have at least 20 data points."
        )
        quit()
    ...
    if manual_init is not None:
        try:
            if len(manual_init) == 4:
                ...
                print(manual_init)
                print(
                    ">Manual input wrong format, please follow the correct "
            ...
            print(
                "Problem loading manually initialized parameters, please "
    ...
    )  # xdata is new set of data to be fit, within extract_factor
    ...
    try:
        ...
        if (
            ...
            params.add(
                "Q", value=init[0], vary=change_Q, min=init[0] * 0.3, max=init[0] * 2
            )
        ...
        params.add(
            "Qc", value=init[1], vary=change_Qc, min=init[1] * 0.5, max=init[1] * 1.5
        )
        params.add(
            "w1", value=init[2], vary=change_w1, min=init[2] * 0.9, max=init[2] * 1.1
        )
        if Method.method == "CPZM":
            params.add(
                "Qa",
        ...
        print(f"Exception {e}")
        print(
            ">Failed to define parameters, please make sure parameters are of correct format"
        )
        quit()
    ...
    if output_params[0] != fit_params[0]:
        ...
        if (
            ...
            params2.add(
                ...
                value=output_params[0],
                vary=change_Qi,
                min=output_params[0] * 0.8,
                max=output_params[0] * 1.2,
            )
        params2.add(
            "Qc",
            value=output_params[1],
            vary=change_Qc,
            min=output_params[1] * 0.8,
        ...
        params2.add(
            ...
            vary=change_w1,
            min=output_params[2] * 0.9,
            max=output_params[2] * 1.1,
        )
        if Method.method == "CPZM":
            params2.add(
                "Qa",
                value=output_params[3],
                vary=change_Qa,
                min=output_params[3] * 0.9,
    ...
    if len(xdata) == 0:
        if manual_init is not None:
            print(
                ">Length of extracted data equals zero thus bandwidth is incorrect, "
                "most likely due to initial parameters being too far off"
            )
            print(
                ">Please enter a new set of manual initial guess data "
