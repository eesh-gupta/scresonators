...
def fit_resonator(
    data, filename, output_path, fit_type="DCM", plot=False, pre="circle"
):
    # fit type DCM, CPZM
...
def grab_data(pth, fname, meas_type="vna", slope=0):
    ...
    if meas_type == "vna":
        ...
        # data["phases"][0] = np.unwrap(data["phases"][0])

        # slope = (data['phases'][0][5] - data['phases'][0][0]) / (data['xpts'][0][5] - data['xpts'][0][0])
        print(slope)
        if (
            #data["phases"][0][-1] < data["phases"][0][0] or
            data["phases"][0][-1] < data["phases"][0][-2]
        ):
            data["phases"] = -data["phases"][0] - slope * data["xpts"][0]
        else:
            data["phases"] = data["phases"][0] - slope * data["xpts"][0]
        data["phases"] = np.unwrap(data["phases"])
        # data["phases"] = data["phases"] - data["phases"][0]

        # inds = data["phases"] < -np.pi
        # data["phases"][inds] = np.mod(data["phases"][inds], np.pi)
        # inds = data["phases"] > np.pi / 2
        # data["phases"][inds] = np.mod(data["phases"][inds], np.pi / 2)
        # inds = data["phases"] < -np.pi / 2
        # data["phases"][inds] = -np.mod(data["phases"][inds], np.pi / 2)
        data["lin"] = data["amps"]
...
def plot_raw_data(data, phs_off=0, amp_off=0):
    ...
    plt.plot(amp / max_amp * np.cos(phs), amp / max_amp * np.sin(phs), ".")
            ...
            for k in range(len(file_list)):
                ...
                if norm:
                    ...
                    z = scfit.preprocess_circle(
                        data["freqs"], x + 1j * y, output_path="hi", plot_extra=False
                    )
                    ax[j].plot(
