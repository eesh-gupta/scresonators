...
def grab_data(pth, fname, meas_type='vna', slope=0):
    ...
    if meas_type=='vna':
        ...
        print(slope)
        if data['phases'][0][-1] < data['phases'][0][0] and  data['phases'][0][-1] < data['phases'][0][-2]:
...
def analyze_sweep_gen(directories, pth_base, img_pth, nfiles=3, name='res', plot=False, min_power=-120, type='vna', slope=0, fix_freq=False):
    if nfiles==3:
        if type=='vna': 
            pattern_end = 'dbm_'
            ends = ['wide1', 'narrow', 'wide2']
