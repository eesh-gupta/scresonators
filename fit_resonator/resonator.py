...
class FitMethod(object):
    ...
    def __init__(
        ...
        assert method in [
            ...
            "INV",
            "CPZM",
        ], "Wrong Method, please input:PHI, DCM, INV or CPZM"
        assert (manual_init == None) or (
            type(manual_init) == list and len(manual_init) == 4
        ), "Wrong manual_init, None or len = 6"
        self.method = method
        if method == "DCM":
...
class Resonator:  # Object is auto-initialized with @attr annotation
        ...
        if self.method is None:
            ...
            if method == "INV":
                ...
                self.compare = ff.fit_raw_compare(
                    self.data.freqs, self.data.amps, self.INVparams.all, method
                )
            elif method == "CPZM":
                ...
                self.compare = ff.fit_raw_compare(
                    self.data.freqs, self.data.amps, self.DCMparams.all, method
                )
    ...
    def reload_params(self, method: str, params: np.ndarray, chi):
        ...
        if method in self.method:
            ...
            if method == "DCM REFLECTION":
                self.DCMparams = DCMparams(params, chi)
                self.compare = ff.fit_raw_compare(
                    self.data.freqs, self.data.amps, self.DCMparams.all, "DCM"
                )
            elif method == "INV":

                self.INVparams = INVparams(params, chi)
                self.compare = ff.fit_raw_compare(
                    self.data.freqs, self.data.amps, self.INVparams.all, "INV"
                )
            elif method == "CPZM":
    ...
    def power_calibrate(self, p):
        ...
        assert (self.DCMparams is not None) or (
            self.INVparams is not None
        ), "Please load parameters first"
        p = np.array(p)
        x = self.power
        f = self.fc
        ...
        if "DCM" in self.method:
            ...
            self.DCMparams.num_photon = (
                2 * p / hbar / f**2 * Q**2 / Qc
            )  ### calculate number of photon
...
def from_file(filepath, data_column=None, fscale=1):
    ...
    if extension.startswith(".s") and extension.endswith("p"):
        try:
            ...
            quit()
        file, inline, options, frequency_units, data_format = header_parse(
            file=snp_file
        )
        ...
        try:
            ...
            file, line, options, frequency_units, data_format = header_parse(
                file=txt_file
            )
            data_lines = []
            while line:
            ...
            print(
                f"Exception: **{e}** encountered when attempting to load data file as .txt/.csv"
            )
...
def header_parse(file):
    ...
    while (
        any(comment in inline.lower() for comment in comment_line)
        or any(option_lead in inline for option_lead in option_line)
        or not any(number in inline.lower()[0] for number in nums)
    ):
        if (
            any(option_lead in inline.lower() for option_lead in option_line)
        ...
        if isinstance(s_col, int):
            ...
            for metadata in options:
                if "Measurements: " in metadata:
                    measurements = (
                        metadata.rsplit("Measurements: ")[1]
                        .strip(".:\n")
                        .lower()
                        .split(", ")
                    )
